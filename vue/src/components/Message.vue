...
<template>
  <div :class="[top ? 'mt-2' : 'mt-0.5', 'mx-4 relative flex', { 'flex-row-reverse': user }]">
    <div v-if="bottom"
      :class="[' absolute bottom-0 overflow-hidden', 'h-4 w-4.5',
        user ? 'bg-sky-500 dark:bg-sky-700 right-0 -mr-3 rounded-bl-50' 
        : 'bg-gray-200 dark:bg-zinc-700 left-0 -ml-3 rounded-br-50']">
      <div :class="['h-5.5 -mt-1 w-4.5 bg-white dark:bg-zinc-900',
        user ? 'ml-1.5 rounded-bl-50' : '-ml-1.5 rounded-br-50']">
      </div>
    </div>
    <div :class="['px-2.5 py-1 rounded-2xl break-words overflow-hidden font-sans whitespace-pre-line', 
